...
causal_quartet_time |>
  nest_by(dataset) |>
  mutate(`ATE not adjusting for Z` = round(coef(lm(outcome_followup ~ exposure_baseline, data = data))[2], 2),
         `ATE adjusting for Z` = round(coef(lm(outcome_followup ~ exposure_baseline + covariate_baseline, data = data))[2], 2)) |>
